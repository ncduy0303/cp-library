// Source: https://github.com/nilehmann/wavelet-tree/blob/master/wavelet-tree.cpp
...
struct BitmapRank {
    ...
    void set(int i, bool b) { set_bit(vec[i / bits], i % bits, b); }
    void build_rank() {
        for (int i = 1; i < (int)vec.size(); ++i) {
            count[i] = count[i - 1] + popcnt(vec[i - 1]);
        }
    }
    int rank1(int i) const { return i < 0 ? 0 : count[i / bits] + popcnt(vec[i / bits] << (bits - i % bits - 1)); }
    int rank1(int i, int j) const { return rank1(j) - rank1(i - 1); }
    int rank0(int i) const { return i < 0 ? 0 : i - rank1(i) + 1; }
    int rank0(int i, int j) const { return rank0(j) - rank0(i - 1); }
};
...
struct WaveMatrixSucc {
    ...
    WaveMatrixSucc(vector<int> &A) : WaveMatrixSucc(A, *max_element(A.begin(), A.end()) + 1) {}

    // sigma = size of the alphabet, ie., one more than the maximum element in A.
    WaveMatrixSucc(vector<int> &A, int sigma) : height(log2(sigma - 1)), B(height), z(height) {
        for (uint l = 0; l < height; ++l) {
            ...
            B[l].build_rank();
            auto it = stable_partition(A.begin(), A.end(), [=](int c) { return not get_bit(c, height - l - 1); });
    ...
    int range(int i, int j, int a, int b, int L, int U, int l) const {
        ...
        if (a <= L && U <= b) {
            ...
            int left = range(B[l].rank0(i - 1), B[l].rank0(j) - 1, a, b, L, M, l + 1);
            int right = range(z[l] + B[l].rank1(i - 1), z[l] + B[l].rank1(j) - 1, a, b, M + 1, U, l + 1);
            return left + right;
        }
    }
};